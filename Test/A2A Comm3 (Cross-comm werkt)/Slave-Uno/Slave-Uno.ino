...
void requestEvent() {
  ...
  String text = "m:";
  text.concat(counter100);
  text.concat(counter10);
  text.concat(counter1);
  text.concat("\n");

  Serial.println(text);
  
  char charArray[7];
  text.toCharArray(charArray, 7);
  Wire.write(charArray); // respond with message of 6 bytes
...
//Method to call when Arduino received an C
void receivedC(String remainderMessage){

  char nextChar = remainderMessage.charAt(0);       //Select the first char
  remainderMessage = remainderMessage.substring(1); //Remove first char from message
  Serial.print("receivedC() received parameters: ");
  Serial.println(remainderMessage);
